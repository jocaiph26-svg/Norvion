...
def settings_page(request: Request, run_id: Optional[int] = Query(None)):
    ...
    return _render_or_fallback(
        ...
        fallback_html=(
            "<p>Settings template missing. Use <code>POST /settings</code> to update.</p>"
...
def settings_save(
    ...
    tenant_id = _tenant_id(request)
    existing = read_settings(tenant_id)
    ...
    new_s = {
        "config_version": existing.get("config_version", CONFIG_VERSION),
        ...
        "webhook_secret": existing.get("webhook_secret", "CHANGE_ME_DEMO_SECRET"),
    }
    new_hash = _canonical_json_hash(new_s)
    write_tenant_settings(tenant_id, new_s)
    _log_access(
        tenant_id,
        _actor_id(request),
        _access_role(request),
        "update",
        f"settings_scope:tenant:{new_hash}",
    )
...
def alert_detail(request: Request, alert_id: str, readonly: bool = Query(False), run_id: Optional[int] = Query(None)):
    ...
    return _render_or_fallback(
        ...
        "alert_detail.html",
        {
            ...
            "audit_allowed": bool(_role_rank(role) >= _role_rank("admin")),
...
def insights(request: Request, run_id: Optional[int] = Query(None)):
    _log_access(_tenant_id(request), _actor_id(request), _access_role(request), "view", "insights")
    active_run, latest_run_actual = _active_and_latest(request, run_id)
    ...
    """
            ...
            if _table_has_column(conn, "runs", "tenant_id"):
                rows = conn.execute(
                    "SELECT id, created_at, summary_json, alerts_json FROM runs WHERE id <= ? AND COALESCE(tenant_id, ?) = ? ORDER BY id DESC LIMIT 2",
                    (active_id, TENANT_DEFAULT, tenant_id),
                ).fetchall()
            else:
                rows = conn.execute(
    ...
    quality = None
    try:
        if latest and ("quality_json" in latest.keys()) and latest["quality_json"]:
    ...
    latest_alerts_list = safe_json_loads(latest["alerts_json"], []) or []
    latest_alerts = {str(a.get("id")) for a in latest_alerts_list if isinstance(a, dict)}
    ...
    insights_list = _insight_narratives(
        latest_summary,
        [a for a in latest_alerts_list if isinstance(a, dict)],
        quality,
        runway_na,
        runway_note,
    )
...
def unified_dashboard(request: Request, tab: str = "overview", run_id: Optional[int] = Query(None)):
    ...
    if not latest_run:
        return _render_or_fallback(
            ...
            "dashboard_unified.html",
            {
                ...
                "latest_run": None,
                "alerts_overview": [],
                "digest": {"latest_run_id": None, "prev_run_id": None, "new_alerts": [], "cleared_alerts": [], "delta": {}},
                "unresolved": [],
                "week_events": [],
                "active_run_id": None,
                "latest_run_id": None,
    ...
    with db_conn() as conn:
        ...
        for r in alert_rows:
            item = dict(r)
            item["alert_id"] = _strip_tenant_alert_id(tenant_id, str(item.get("alert_id") or ""))
            alerts_overview.append(item)

        if _table_has_column(conn, "runs", "tenant_id"):
            runs2 = conn.execute(
                "SELECT id, created_at, summary_json, alerts_json FROM runs WHERE COALESCE(tenant_id, ?) = ? ORDER BY id DESC LIMIT 2",
                (TENANT_DEFAULT, tenant_id),
            ).fetchall()
        else:
            runs2 = conn.execute(
...
def view_run(request: Request, run_id: int):
    ...
    return _render_or_fallback(
        ...
        "dashboard.html",
        {
            ...
            "run_qs": f"?run_id={int(run_id)}" if (_get_latest_run_id(tenant_id) and int(run_id)!=int(_get_latest_run_id(tenant_id))) else "",
        },
...
def api_integration_sync_now(request: Request, provider: str):
    ...
    try:
        if candidate.exists():
            # ingest local file
            resp = api_ingest_local(request=request, path=candidate.name, provider=p)
            ...
            resp = api_ingest_simulate(request=request, seed=42, days=120, rows_per_day=8, provider=p)
            if isinstance(resp, JSONResponse):
                data = safe_json_loads(resp.body, {}) if hasattr(resp, "body") else {}
                run_id = int(data.get("run_id")) if isinstance(data, dict) and data.get("run_id") is not None else None
